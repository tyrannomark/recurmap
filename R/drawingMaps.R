...
# ##
# ## ### ##### ######## ############## ######################
addPng <- function(ll,rfilepath) {
  l <- ll
  l$G <- function(d) l$G(d) + theme(text=element_text(size=32))
  l$run <- function(d) {
    ll$run(d)
    ggsave(rfilepath,device="png",width=2048,height=2048,units="px")
  }
  l
}
test.addPng <- function() {
  data.frame(t1=(0:100),v1=sin((0:100)/3*pi)) %>%
    ...
    (function(d) { d$delta <- (d$v2-d$v1) ** 2; d } ) -> d
  (list()) %>% buildGraph() %>% addPng("test-saved-image.png") -> l
  (d) %>% l$run()
}
test.addPng()
# ## ### ##### ######## ############## ######################
...
#
## Add field to a data.frame, value by function applied to each row
...
# ## ### ##### ######## ############## ######################
baseMap <- function(postProcessing=NULL) {
  function(data) {
    if (is.null(postProcessing)) { postProcessing <- function(x) x }
    (ggplot() +
        geom_tile( data=data, aes(x=t1,y=t2,fill=delta ) ) +
        theme_bw() +
        scale_x_continuous() +
        scale_y_continuous()
    ) %>%
...
# ## ### ##### ######## ############## ######################
# ## ### ##### ######## ############## ######################
# ## Make a recurrence data set given a data frame and comparator
# ##   f is a function finalising the graph drawing
# ## ### ##### ######## ############## ######################
...
tgfile <- "/Users/marke/Google\ Drive/@@Projects/2022/2022-Igu-ritual-Keraa/01-Data/1-Kaliwu-20190804/keraa-20190804-UR-01-G-kaliwu.TextGrid"
tg <- TextGrid(tgfile)
...
lcSuffix(c("dogmatix","felix"))


comparator <- function(a,b) { ifelse(a$t == b$t, 0.0, str_length(lcSuffix(c(a$v,b$v)))) }
comparator <- function(a,b) { ifelse(a$t == b$t, 0.0, str_length(longestCommonSuffix(c(a$v,b$v))))}
...
filepath <- "/Users/marke/Google\ Drive/@@Projects/2022/2022-Igu-ritual-Keraa/01-Data/1-Kaliwu-20190804/keraa-20190804-UR-01-G-kaliwu-01.wav"
filepath %>%
